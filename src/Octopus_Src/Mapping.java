...
public class Mapping {
	...
	public void Indexing(){
		...
		if(!cf.checkReference("fasta",ds.getGSMInfo()[1])){ // exist:true, Not exist:false
		...
		if (aligntool.equals("Hisat2")) {
			...
			String starIdx_Cmd[] = { path + "Tools/STAR/bin/Linux_x86_64/STAR","--runMode","genomeGenerate","--genomeDir",path+"Index/STAR/"+genome,"--genomeFastaFiles",path+"Index/Reference/"+genome+".fasta",
					"--runThreadN",ds.getAnalysisCPU(),"--sjdbOverhang","100","--sjdbGTFfile",path+"Index/Reference/"+genome+".gtf"};

			
			String writeCmd = path + "Tools/STAR/bin/Linux_x86_64/STAR --runMode genomeGenerate --genomeDir "+path+"Index/STAR/"+genome+" --genomeFastaFiles "+path+"Index/Reference/"+genome+".fasta --runThreadN " +ds.getAnalysisCPU()+" --sjdbOverhang 100 --sjdbGTFfile "+path+"Index/Reference/"+genome+".gtf";
		...
		try{
			...
			if(aligntool.equals("Hisat2")){
				...
				if(fr.endsWith(".gz")){
					if(ds.getFullOption()){
						if (ds.getReverseFastq().equals("")) {
							align_Cmd = spacePath + "Tools/STAR/bin/Linux_x86_64/STAR --genomeDir "+ spacePath +"Index/STAR/"+genome+"/ --runThreadN "+ds.getAnalysisCPU()+" "+strand+" --outSAMtype BAM Unsorted "+fullOption+" --outFileNamePrefix "+bamPath.replace(" ", "\\ ") + fileName + " --readFilesIn <(zcat "+ ds.getForwardFastq().replace(" ", "\\ ")+")";
						} else {
							align_Cmd = spacePath + "Tools/STAR/bin/Linux_x86_64/STAR --genomeDir "+ spacePath +"Index/STAR/"+genome+"/ --runThreadN "+ds.getAnalysisCPU()+" "+strand+" --outSAMtype BAM Unsorted "+fullOption+" --outFileNamePrefix "+bamPath.replace(" ", "\\ ") + fileName + " --readFilesIn <(zcat "+ ds.getForwardFastq().replace(" ", "\\ ") + ") <(zcat "+ds.getReverseFastq().replace(" ", "\\ ")+")";						
						}			
					}else{
						if (ds.getReverseFastq().equals("")) {
							align_Cmd = spacePath + "Tools/STAR/bin/Linux_x86_64/STAR --genomeDir "+ spacePath +"Index/STAR/"+genome+"/ --runThreadN "+ds.getAnalysisCPU()+" "+strand+" --outSAMtype BAM Unsorted --outFileNamePrefix "+bamPath.replace(" ", "\\ ") + fileName + " --readFilesIn <(zcat "+ ds.getForwardFastq().replace(" ", "\\ ")+")";
						} else {
							align_Cmd = spacePath + "Tools/STAR/bin/Linux_x86_64/STAR --genomeDir "+ spacePath +"Index/STAR/"+genome+"/ --runThreadN "+ds.getAnalysisCPU()+" "+strand+" --outSAMtype BAM Unsorted --outFileNamePrefix "+bamPath.replace(" ", "\\ ") + fileName + " --readFilesIn <(zcat "+ ds.getForwardFastq().replace(" ", "\\ ") + ") <(zcat "+ds.getReverseFastq().replace(" ", "\\ ")+")";						
						}
					}
				} else {
					if(ds.getFullOption()){
						if (ds.getReverseFastq().equals("")) {
							align_Cmd = spacePath + "Tools/STAR/bin/Linux_x86_64/STAR --genomeDir "+ spacePath +"Index/STAR/"+genome+"/ --runThreadN "+ds.getAnalysisCPU()+" "+strand+" --outSAMtype BAM Unsorted "+fullOption+" --outFileNamePrefix "+bamPath.replace(" ", "\\ ") + fileName + " --readFilesIn "+ ds.getForwardFastq().replace(" ", "\\ ");
						} else {
							align_Cmd = spacePath + "Tools/STAR/bin/Linux_x86_64/STAR --genomeDir "+ spacePath +"Index/STAR/"+genome+"/ --runThreadN "+ds.getAnalysisCPU()+" "+strand+" --outSAMtype BAM Unsorted "+fullOption+" --outFileNamePrefix "+bamPath.replace(" ", "\\ ") + fileName + " --readFilesIn "+ ds.getForwardFastq().replace(" ", "\\ ") + " "+ds.getReverseFastq().replace(" ", "\\ ");						
						}
					}else{
						if (ds.getReverseFastq().equals("")) {
							align_Cmd = spacePath + "Tools/STAR/bin/Linux_x86_64/STAR --genomeDir "+ spacePath +"Index/STAR/"+genome+"/ --runThreadN "+ds.getAnalysisCPU()+" "+strand+" --outSAMtype BAM Unsorted --outFileNamePrefix "+bamPath.replace(" ", "\\ ") + fileName + " --readFilesIn "+ ds.getForwardFastq().replace(" ", "\\ ");
						} else {
							align_Cmd = spacePath + "Tools/STAR/bin/Linux_x86_64/STAR --genomeDir "+ spacePath +"Index/STAR/"+genome+"/ --runThreadN "+ds.getAnalysisCPU()+" "+strand+" --outSAMtype BAM Unsorted --outFileNamePrefix "+bamPath.replace(" ", "\\ ") + fileName + " --readFilesIn "+ ds.getForwardFastq().replace(" ", "\\ ") + " "+ds.getReverseFastq().replace(" ", "\\ ");						
						}
					}
				}
			}
	...
	public void Sorting(){
		...
		try {
			String samtoolSort_Cmd[] = {path+"Tools/Samtools/samtools","sort",ds.getBamFile(),"-@",ds.getAnalysisCPU(),"-o",bamPath+"sorted_"+fileName+".bam"};
			String samtoolIndex_Cmd[] = {path + "Tools/Samtools/samtools","index",bamPath+"sorted_"+fileName+".bam"};

			cf.shellCmd(samtoolSort_Cmd);
			cf.shellCmd(samtoolIndex_Cmd);
			
			//Log
			ds.writeLogCmd(path+"Tools/Samtools/samtools sort "+ds.getBamFile()+" -@ "+ds.getAnalysisCPU()+" -o "+bamPath+"sorted_"+fileName+".bam\n");
	...
	public void bamToCram(){
		String bamToCram_Cmd[] = {path+"Tools/Samtools/samtools","view","-C","-T",path+"Index/Reference/"+genome+".fasta",ds.getBamFile(),"-@",ds.getAnalysisCPU(),"-o",bamPath+fileName+".cram"};
